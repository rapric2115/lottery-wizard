...
const GeneradorFormula = () => {
    ...
    const results = useMemo(() => [leidsa[0], leidsa[1], leidsa[2], leidsa[3], leidsa[4], leidsa[5]], [leidsa])
    ...
    useEffect(() => {
        ...
        const calculateCombinations = (leidsaIndex, resultsIndex,arrayNums, aveNums, inicio, fin) => {
          const calculatedCombinations = [];
          for (const value of arrayNums) {
              const result = log(value, Number(leidsaIndex), Number(resultsIndex));
              const logNumber = (aveNums * Number(leidsaIndex) * Number(resultsIndex)) / Math.pow(result, 2);
              const results = Math.abs(Math.round((Math.log(logNumber) * 38) / (Math.pow(720, Math.pow(-sum(leidsaIndex, resultsIndex), -1)))));
        ...
        setCombinacionOne(calculateCombinations(leidsa[0], lastRes[0], OneNumber, OneAverage, 0, 22));
        setCombinacionTwo(calculateCombinations(leidsa[1], lastRes[1], TwoNumber, twoAverage, 2, 29));
        setCombinacionTres(calculateCombinations(leidsa[2], lastRes[2], ThreeNumber, ThreeAverage, 3, 33));
        setCombinationCuatro(calculateCombinations(leidsa[3], lastRes[3], fourNumber, fourAverage, 4, 34));
        setCombinationCinco(calculateCombinations(leidsa[4], lastRes[4], fiveNumber, fiveAverage, 12, 37));
        setCombinationSeis(calculateCombinations(leidsa[5], lastRes[5], sixNumber, sixAverage, 19, 38));
